    ...
    const onSubmit =async(data) => {
        ...
        if(picture){
           await uploadImage().then(async(response)=>{
                let res=await dispatch(registerUser({...data,picture:response.secret_url}));
                if (res?.payload?.user) {
                    navigate("/");
                  }   
            })
...
return (
    ...
    <p className="mt-2 text-sm">Sign up</p>
       ...
       {/* IF ERROR EXISTS */}
        ...
        {status=="loading" ? <PulseLoader color="#fff" size={12} />:"SignUp" }
        </button>
        </form>
        {/* Signin link */}
        <p className="flex flex-col items-center justify-center text-center mt-10 text-md dark:text-dark_text_1">
            <span>Already have an account?</span>
            <Link href="/login" className="hover:underline cursor-pointer transition ease-in duration-300">SignIn</Link>
        </p>
    </div>
    </div>
);
